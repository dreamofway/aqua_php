
  <div class="container">
    <ul class="location">
  		<li><a href="/" class="location_list">홈 </a></li>
  		<li><a class="location_list">교회 소식</a></li>
  		<li><a class="location_list">소식 알림</a></li>
  	</ul>
    <div class="content">
      <div class="content_head">
  			<h2 class="page_title">소식 알림</h2>
        <p class="page_info">
          하나님의 사랑과 구원의 소식을 전파하는
          강남성서침례교회<br />
          GangNam Bible Baptist Church !
  			</p>
  		</div>
      <div class="content_box">
        <?=$loc->draw(); ?>
        <div class="content_box_right">


          <div class="txt_wrap">
            <ul class="sermon_desc">
              <li>
                <span class="highlight">190511 - 강남성서침례교회 소식</span>
              </li>
              <li>
                [날짜]<span>2019.04.13</span>
              </li>
            </ul>
            <div class="txt_script_wrap">
              <p class="txt_script">
                <ol class="decimal">
                  <li>
                    오늘 말씀을 증거해주신 김상열 목사님께 감사를 드립니다. <br />
                    천연숙 사모님의 암 완치와 목사님의 말씀 사역을 위해 기도해 주시기 바랍니다
                  </li>
                  <li>
                    가정의 달, 5월 19일(주일 나들이 친교) 소래포구 - 예배 후에 성도님들과 함께 나들이 친교를 하고자 합니다.<br />
                    성도님들께서 기도하시고 함께 동참하시기를 바랍니다
                  </li>
                  <li>
                    윤효정 자매 가정이 지방에 계신 부모님을 방문 중에 있습니다
                  </li>
                  <li>
                    풍성한 삶(5-6월호, 민수기)가 출간되었습니다.<br />
                    말씀과 함께 풍성한 삶을 누리시기 바랍니다
                  </li>
                </ol>
              </p>
            </div>
          </div>

          <div class="table_wrap">
            <table class="wrsp_tbl">
              <caption class="blind">칼럼, 간증, 큐티</caption>
              <colgroup>
                <col style="width: 80px">
                <col style="width: auto;">
                <col style="width: 200px;">
              </colgroup>
              <thead>
                <tr>
                  <th scope="col">번호</th>
                  <th scope="col">제목</th>

                  <th scope="col">날짜</th>
                </tr>
              </thead>
              <tbody>
                <tr>
                  <td>15</td>
                  <td><a href="#"><p class="title ellipsis on">190511 - 강남성서침례교회 소식</p></a></td>

                  <td>2019.04.13</td>
                </tr>
                <tr>
                  <td>14</td>
                  <td><a href="#"><p class="title ellipsis">190511 - 강남성서침례교회 소식</p></a></td>

                  <td>2019.04.13</td>
                </tr>
                <tr>
                  <td>13</td>
                  <td><a href="#"><p class="title ellipsis">190511 - 강남성서침례교회 소식</p></a></td>

                  <td>2019.04.13</td>
                </tr>
                <tr>
                  <td>12</td>
                  <td><a href="#"><p class="title ellipsis">190511 - 강남성서침례교회 소식</p></a></td>

                  <td>2019.04.13</td>
                </tr>
                <tr>
                  <td>11</td>
                  <td><a href="#"><p class="title ellipsis">190511 - 강남성서침례교회 소식</p></a></td>

                  <td>2019.04.13</td>
                </tr>
                <tr>
                  <td>10</td>
                  <td><a href="#"><p class="title ellipsis">190511 - 강남성서침례교회 소식</p></a></td>

                  <td>2019.04.13</td>
                </tr>
                <tr>
                  <td>9</td>
                  <td><a href="#"><p class="title ellipsis">190511 - 강남성서침례교회 소식</p></a></td>

                  <td>2019.04.13</td>
                </tr>
                <tr>
                  <td>8</td>
                  <td><a href="#"><p class="title ellipsis">190511 - 강남성서침례교회 소식</p></a></td>

                  <td>2019.04.13</td>
                </tr>
                <tr>
                  <td>7</td>
                  <td><a href="#"><p class="title ellipsis">190511 - 강남성서침례교회 소식</p></a></td>

                  <td>2019.04.13</td>
                </tr>
                <tr>
                  <td>6</td>
                  <td><a href="#"><p class="title ellipsis">190511 - 강남성서침례교회 소식</p></a></td>

                  <td>2019.04.13</td>
                </tr>
                <tr>
                  <td>5</td>
                  <td><a href="#"><p class="title ellipsis">190511 - 강남성서침례교회 소식</p></a></td>

                  <td>2019.04.13</td>
                </tr>
                <tr>
                  <td>4</td>
                  <td><a href="#"><p class="title ellipsis">190511 - 강남성서침례교회 소식</p></a></td>

                  <td>2019.04.13</td>
                </tr>
                <tr>
                  <td>3</td>
                  <td><a href="#"><p class="title ellipsis">190511 - 강남성서침례교회 소식</p></a></td>

                  <td>2019.04.13</td>
                </tr>
                <tr>
                  <td>2</td>
                  <td><a href="#"><p class="title ellipsis">190511 - 강남성서침례교회 소식</p></a></td>

                  <td>2019.04.13</td>
                </tr>
                <tr>
                  <td>1</td>
                  <td><a href="#"><p class="title ellipsis">190511 - 강남성서침례교회 소식</p></a></td>

                  <td>2019.04.13</td>
                </tr>
              </tbody>
            </table>
            <ul class="pagination justify-content-center">
              <li class="page-item disabled"><a href="" class="page-link"><i class="fas fa-angle-double-left"></i></a></li>
              <li class="page-item disabled"><a href="" class="page-link"><i class="fas fa-angle-left"></i></a></li>
              <li class="page-item active"><a href="" class="page-link">1</a></li>
              <li class="page-item"><a href="" class="page-link">2</a></li>
              <li class="page-item"><a href="" class="page-link"><i class="fas fa-angle-right"></i></a></li>
              <li class="page-item"><a href="" class="page-link"><i class="fas fa-angle-double-right"></i></a></li>
            </ul>
            <ul class="search">
              <li>
                <select>
                  <option value="title">제목</option>
                  <option value="contents">본문</option>
                  <option value="pastor">설교자</option>
                </select>
              </li>
              <li>
                <input type="text" required="required" label="검색어"/>
                <button class="btn_search"><i class="fas fa-search"></i></button>
              </li>
            </ul>
          </div>
        </div>
      </div>
    </div>

  </div>
  
